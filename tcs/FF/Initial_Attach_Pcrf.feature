@initial-attach-pcrf @2-attach-procedure-for-eps-services @23401-4g
...
Feature: Attach procedure for EPS services
  ...
  Scenario: Initial Attach with IMSI and PDN Connectivity Request
    ...
    When I send DIAMETER message DIA_CAPABILITIES_EXCHANGE_REQUEST on interface GX with the following details from node PGW1 to PCRF1:
      | parameter                                          | value                                  |
    ...
    Then I receive and validate DIAMETER message DIA_CAPABILITIES_EXCHANGE_REQUEST on interface GX with the following details on node PCRF1 from PGW1:
      | parameter                                          | value                                               |
    ...
    When I send DIAMETER message DIA_CAPABILITIES_EXCHANGE_ANSWER on interface GX with the following details from node PCRF1 to PGW1:
      | parameter                                          | value                                  |
    ...
    Then I receive and validate DIAMETER message DIA_CAPABILITIES_EXCHANGE_ANSWER on interface GX with the following details on node PGW1 from PCRF1:
      | parameter                                          | value                                                |
      | Hop-by-Hop Identifier                              | save(HOP_BY_HOP_ID)                                  |
      | End-to-End Identifier                              | save(END_TO_END_ID)                                  |
    ...
    When I send S1AP message S1_DOWNLINK_NAS_SECURITY_MODE_CMD on interface S1-MME with the following details from node MME1 to eNodeB1:
      ...
      | nas_pdu.security_mode_command.imeisv_request              | 1                                      |
    ...
    When I send S1AP message S1_UPLINK_NAS_SECURITY_MODE_COMPLETE on interface S1-MME with the following details from node eNodeB1 to MME1:
      ...
      | nas_pdu.security_mode_complete.mobile_identity.imei_sv | 9900008624718543                       |
      | eutran_cgi.plmn_identity.mcc                           | {abotprop.SUT.MCC}                     |
      | eutran_cgi.plmn_identity.mnc                           | {abotprop.SUT.MNC}                     |
      | eutran_cgi.cell_id                                     | {abotprop.SUT.CGI.CELL.ID}             |
      | tai.plmn_identity.mcc                                  | {abotprop.SUT.MCC}                     |
    ...
    When I send S1AP message S1_INIT_CTXT_SET_RES on interface S1-MME with the following details from node eNodeB1 to MME1:
      ...
      | erab_setup_list.0.transport_layer_address | {abotprop.SUT.DEF.ERAB.TRANSPORT.LAYER.ADDR} |
    ...
    Then I receive and validate S1AP message S1_INIT_CTXT_SET_RES on interface S1-MME with the following details on node MME1 from eNodeB1:
      ...
      | erab_setup_list.0.transport_layer_address | {string:eq}({abotprop.SUT.DEF.ERAB.TRANSPORT.LAYER.ADDR}) |
